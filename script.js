document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkoutAmountElements = document.querySelectorAll('#checkoutAmount, #checkoutAmount2');
    const currencySelect = document.getElementById('currency-select');
    const transfermovilInfoContainer = document.getElementById('transfermovil-info');
  
    const conversionRate = 250; // Ejemplo de tasa de conversión de 1 MLC = 250 CUP
    ...
    // Función para renderizar la información de pago según la moneda seleccionada
    function renderPaymentInfo(currency) {
      ...
      if (currency === 'cup') {
        if (transfermovilInfoContainer) {
          transfermovilInfoContainer.innerHTML = `
            <p><strong>Número de Teléfono:</strong> +53 53932292</p>
            <p><strong>Tarjeta CUP:</strong> 9238129975083223</p>
          `;
        }
        checkoutAmountElements.forEach(el => el.textContent = `$${total.toLocaleString()} CUP`);
        ...
        checkoutAmountElements.forEach(el => el.textContent = `$${totalMLC} MLC`);
      }
    }
    ...
    function init() {
      ...
      // Renderiza la información de pago en la página de checkout al cargar
      if (document.body.classList.contains('checkout-page') && currencySelect) {
        renderPaymentInfo(currencySelect.value);
      }
